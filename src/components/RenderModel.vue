...
export default {
  ...
  render(createElement) {
    ...
    function getDescription(model) {
      ...
      if (model) {
        if (model.reference) {
          ...
          node.push(createElement("span", [getDescription(model.reference), ':']));
        }
        if (model.id) {
        ...
        if (model.selector) {
          node.push(createElement("span", ['(', getDescription(model.selector), ')']));
        }
    ...
    const counter = createElement('span', ["one", createElement('span', ["two"])]);
    ...
    var inputText2 = createElement('input', {
                domProps: { value: this.model.reference.id}}, this.model.reference.id)
